...
describe('createObjectiveProgress', () => {
  ...
  it('sets target from the objective for the deliver type', () => {
    const board = parseTestBoard(['M O', 'O M']);
    expect(createObjectiveProgress({ type: 'deliver', count: 3 }, board)).toEqual({ current: 0, target: 3 });
  });
});
...
describe('advanceObjective', () => {
  ...
  it('advances deliver progress by big fruit delivered this turn, ignoring unrelated tallies', () => {
    const objective = { type: 'deliver', count: 3 } as const;
    const progress = { current: 1, target: 3 };
    const next = advanceObjective(progress, objective, {
      byFruit: { mango: 5 },
      jellyClearedCount: 2,
      deliveredCount: 1,
  ...
  it('clamps deliver progress at the target and never exceeds it', () => {
    const objective = { type: 'deliver', count: 2 } as const;

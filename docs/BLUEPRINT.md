...
4. **เป้าหมายด่าน 4 แบบ**: เก็บผลไม้ตามชนิด / เคลียร์วุ้น / ทำแต้ม / ส่งผลไม้ลูกใหญ่ลงตะกร้า (deliver — เพิ่มใน M9)
5. **เม็ดพิเศษ**: ลาย (จับ 4), ห่อ (จับ L/T), ระเบิดสายรุ้ง (จับ 5), เม็ดสายฝน (จับกลุ่มเดียว 6 เม็ดขึ้นไป — เพิ่มใน M9) + คอมโบผสมครบ 6 คู่
6. **กระดาน**: สูงสุด 8×8 (เม็ด ≥45px บนมือถือ), เริ่ม 6×6/4 ชนิด ไต่ไป 8×8/6 ชนิด, รองรับรูปทรงเว้าแหว่ง, และตั้งแต่ M9 มีช่องดอกไม้ (จับแล้วบาน แถมตาเดิน) กับช่องตะกร้า (แถวล่างสุด รับผลไม้ลูกใหญ่)
7. **ด่าน**: 80 ด่าน เป็นไฟล์ข้อมูล TS (เติมได้ไม่แตะโค้ด) — 60 ด่านแรกจากแผนเดิม บวก 20 ด่าน (61-80) จาก M9 สอนกลไกใหม่ทีละอย่าง (ดู `docs/BLUEPRINT-M9.md`), ดาว 1-3 (ผ่าน = ≥1 ดาวเสมอ), ย้อนเล่นได้ตลอด
8. **ธีม**: ผลไม้ไทย 6 ชนิด (มะม่วง ส้ม องุ่น แตงโม มังคุด กล้วย) — แยกได้ด้วยรูปทรง ไม่ใช่แค่สี
9. **ควบคุม**: swipe + tap-tap บนมือถือ, ลากเมาส์ + คลิกสองครั้งบนคอม (ผลไม้ลูกใหญ่ยังสลับข้างๆ ได้เสมอแม้ไม่เกิดการจับ — เสีย 1 ตาเดิน, ดู ADR-0006)
...
14. **เซฟ**: localStorage ต่อเครื่อง (ไม่ sync ข้ามเครื่อง — ยอมรับ), สอนเล่นใน 3 ด่านแรก และด่าน 61/65/67/71 (กลไกใหม่ M9), ตั้งค่า: เพลง/SFX/คำใบ้/ความเร็วอนิเมชัน (ปกติ/ช้า)
15. **สวนประจำวัน (M9)**: กระดานสุ่มจาก seed วันที่ 1 กระดาน/วัน ไม่ใช่ด่าน ไม่มีดาว ไม่ปลดล็อกอะไร ย้อนไปเล่นวันเก่าได้ทุกวันไม่จำกัด — เก็บเป็นดอกไม้ในปฏิทินสวน (ดู ADR-0005, ADR-0006)
...
- `Piece { id, fruit: FruitKind|null, special: 'none'|'stripedH'|'stripedV'|'wrapped'|'colorBomb'|'rain', big: boolean }` — id คงที่ให้ renderer ตามเม็ดข้ามการตกได้; `big: true` = ผลไม้ลูกใหญ่ (M9, จับไม่ได้ สลับข้างๆ ได้เสมอ)
- `Cell { kind: 'normal'|'hole', jelly: boolean, flower: boolean, basket: boolean, piece }` (flower/basket เพิ่มใน M9), `Board { width≤8, height≤8, cells[] }`
- `LevelDef { id, layout: string[] ('.'=ปกติ 'X'=ช่องปิด 'J'=วุ้น 'F'=ดอกไม้ 'B'=ตะกร้า), fruits[4-6], moves (บวก buffer ใจดีแล้ว), objective (รวม deliver), bigFruitTotal?, starScores: [2ดาว, 3ดาว], tutorial? }`
- `SaveData { version: 2, unlockedLevel, stars{}, failStreak{levelId,count}, tutorialSeen[], dailyBlooms[], calendarHintSeen, settings{music,sfx,hints,animSpeed} }` — v1→v2 เพิ่ม `dailyBlooms` (M9.4), `calendarHintSeen` เป็นฟิลด์เสริมเพิ่มทีหลังแบบไม่ขึ้นเวอร์ชัน (M9.5)
...
### โค้งความยาก 80 ด่าน (sawtooth นุ่มๆ — ด่านยากตามด้วยด่านง่าย 1-2 ด่านเสมอ; 1-60 คือแผนเดิม, 61-80 เพิ่มใน M9 — รายละเอียดเต็มใน `docs/BLUEPRINT-M9.md`)
...
| 61-64 | 8×8 / 5-6 | สวนที่ 7 (M9): ช่องดอกไม้ — จับแล้วบาน แถมตาเดิน |
| 65-70 | 8×8 / 4-6 | ตะกร้า + ผลไม้ลูกใหญ่ (เป้าหมาย deliver) — สอนสลับลูกใหญ่ข้างๆ แม้ไม่เกิดการจับ |
| 71-75 | 7×7-8×8 / 4-5 | เม็ดสายฝน — จับกลุ่มเดียว 6 เม็ดขึ้นไป กวาดทั้งแถว+คอลัมน์ |
| 76-80 | 6×6-8×8 / 4-5 | ผสมกลไกใหม่ทั้งหมด, ด่าน 80 ฉลองจบง่ายที่สุดในชุด |

move limit ต่อด่าน = ประมาณตาที่ผู้เล่นทั่วไปใช้ × 1.35-1.5 ปัดขึ้น (ด่านที่มีผลไม้ลูกใหญ่เผื่อเพิ่มอีก เพราะการสลับข้างๆ กินตาเดินโดยไม่ทำแต้ม — ดู ADR-0006)
...
เม็ดผลไม้ (Piece), ผลไม้ (Fruit — "สี" ของเกม 6 ชนิด), การจับ (Match ≥3 เรียง), เม็ดพิเศษ (ลาย/ห่อ/ระเบิดสายรุ้ง/สายฝน), คอมโบ (สลับพิเศษ×พิเศษ), ลูกโซ่ (Cascade), วุ้น (Jelly — สถานะติดช่อง), ช่องปิด (Blocked Cell), ช่องดอกไม้ (Flower Cell), ผลไม้ลูกใหญ่ (Big Fruit), ตะกร้า (Basket), ตาเดิน (Move), **Mercy Moves** (+5 ตาหลังแพ้ 2 ครั้งติด), **Gentle Fail** (แพ้แบบไร้บทลงโทษ), ค้อน (Hammer ฟรี 3/ด่าน), คำใบ้ (Hint), ดาว (Stars ≥1 เสมอเมื่อผ่าน), เป้าหมายด่าน (Objective), สวนประจำวัน (Daily Garden), ดอกไม้ประจำวัน (Daily Bloom), ปฏิทินสวน (Garden Calendar) — คำสามข้อท้ายนี้เพิ่มใน M9
...
3. `0003-kindness-model.md` — คง move limit + Gentle Fail + Mercy Moves แทนตัด fail ทิ้ง (กำหนดสมดุลทั้งชุดด่าน — ย้อนยาก)
4. `0004-board-max-8x8.md` — accessibility (เม็ด ≥45px) มาก่อนความจุด่าน
5. `0005-daily-garden.md` (M9) — กระดานสุ่มจาก seed วันที่แทนการเติมด่านอย่างเดียว, SaveData v1→v2
6. `0006-big-fruit-free-swap.md` (M9) — ผลไม้ลูกใหญ่สลับข้างๆ ได้เสมอโดยไม่ต้องเกิดการจับ กันด่าน deliver ตันแบบเงียบ
...
- **M9 ใหญ่ (เสร็จแล้ว)** — แม่เล่นครบ 60 ด่านแล้วเลิกเปิดเกม: เพิ่มสวนประจำวัน (กระดานสุ่มจากวันที่ + ปฏิทินดอกไม้) และกลไกใหม่ 3 อย่าง (ช่องดอกไม้, ตะกร้า+ผลไม้ลูกใหญ่, เม็ดสายฝน) พร้อมด่านใหม่ 61-80 ที่สอนแต่ละอย่างทีละก้อน — รายละเอียดเต็มอยู่ที่ `docs/BLUEPRINT-M9.md`

## Verification

1. **Unit tests (vitest ใน Node)**: helper `parseBoard()` รับ ASCII fixture อ่านง่าย — ครอบคลุม: ทุกรูปแบบ match (3/4H/4V/5/L×4/T×4/+), เอฟเฟกต์+คอมโบพิเศษทุกคู่, cascade หลาย phase แบบ seed คงที่ assert กระดานจบเป๊ะ, สลับผิดไม่กินตา, ค้อนไม่กินตา, gravity รอบช่องปิด, ตรวจกระดานตัน+reshuffle, เป้าหมาย/ดาว/Mercy Moves, validate ด่านทั้ง 80 (layout ถูก, มีทาง refill, threshold เรียงถูก) และ soak test เล่นสุ่มยาวๆ ครอบทุกด่านตรวจ invariant (M9.1c)
